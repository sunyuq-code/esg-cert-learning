...
def test_generation_pipeline(kps, qs, bank):
    ...
    check("T7", "build_prompt包含知识点详解",
          t001["id"] in prompt and "知识点详解" in prompt)
    check("T7", "build_prompt包含去重参考(已有题目)",
          "已有" in prompt)
...
def test_fix_questions(kps, qs, bank):
    ...
    check("T8", "改写提示词包含知识点", kp["id"] in prompt and "知识点详解" in prompt)
    check("T8", "改写提示词包含两道题ID", h017[0]["id"] in prompt and h017[1]["id"] in prompt)
