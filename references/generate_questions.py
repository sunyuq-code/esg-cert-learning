...
def build_prompt(module_name, kp_groups, bank_by_kp):
    """为一批知识点构建生成提示词。"""
    lines = []
    lines.append("你是一位专业的ESG认证考试题库命题专家。请根据给定的知识点内容，生成指定数量和题型的高质量考试题目。")
    lines.append("")
    lines.append("## 题目格式要求")
    lines.append("")
    lines.append("生成一个JSON对象，格式为：")
    lines.append('{"questions": [题目对象, ...]}')
    lines.append("")
    lines.append("题目对象字段：")
    lines.append('- "id": 题目ID(见下方ID规则)')
    lines.append('- "type": "single_choice" | "multiple_choice" | "true_false" | "short_answer"')
    lines.append('- "kp_ref": 知识点ID')
    lines.append('- "level": "basic" | "intermediate" | "advanced"')
    lines.append('- "question": 题干')
    lines.append('- "options": 选项对象，如 {"A":"选项文本","B":"选项文本","C":"选项文本","D":"选项文本"}（判断/简答不需要）')
    lines.append('- "answer": 答案。单选为单个字母如"A"；多选为字母组合如"ABC"；判断为"True"/"False"；简答为"reference_answer"字段')
    lines.append('- "explanation": 解析')
    lines.append('- "reference_answer": 参考答案（仅简答题）')
    lines.append('- "scoring_points": 评分要点列表（仅简答题，3-6个要点）')
    lines.append("")
    lines.append("## ID规则")
    lines.append("ID格式：Q-{KP_ID}-{题型代码}{序号}，题型代码 S=单选 M=多选 T=判断 A=简答。")
    lines.append("例如：Q-ESG-S-001-S4 表示ESG-S-001知识点的第4道单选。")
    lines.append("")
    lines.append("## 题目质量要求")
    lines.append("1. 单选4个选项(A-D)，只有一个正确答案")
    lines.append("2. 多选4-5个选项(A-E)，2-4个正确答案")
    lines.append("3. 判断为对/错陈述，答案True或False")
    lines.append("4. 简答需包含完整参考答案和3-6个评分要点")
    lines.append("5. 题目内容必须严格基于给定的知识点详解，确保答案正确")
    lines.append("6. 考察角度必须与【已有题目】不同，避免题干高度相似")
    lines.append("7. 所有文本使用简体中文，不要使用ASCII双引号作为中文引号（用「」或《》）")
    lines.append("")
    lines.append(f"## 本批知识点（模块 {module_name}）")
    lines.append("")

    for kp, info, need, target in kp_groups:
        lines.append(f"### 知识点: {kp['id']} — {kp.get('topic','')}")
        lines.append(f"模块: {kp.get('module','')}")
        lines.append(f"难度: {kp.get('level','')}")
        lines.append(f"内容类型: {info['ct']} | 复杂度总分: {info['total']} | 目标题量: {target}")
        ...
        for tc in ["S", "M", "T", "A"]:
            if need.get(tc, 0) > 0:
                ...
                lines.append(f"- 需要生成 {need[tc]} 道{type_name}（题型代码{tc}）")
                need_items.append(tc)
        lines.append("")
        # 已有题目(去重参考)
        existing = bank_by_kp.get(kp["id"] + "_questions", [])
        if existing:
            lines.append(f"该知识点已有 {len(existing)} 道题目，新题目的考察角度必须不同：")
...
def call_deepseek(api_key, prompt):
    ...
    system_prompt = (
        "你是ESG认证考试题库生成引擎。严格按照用户要求的JSON格式输出题目数据。"
        "只输出JSON，不要输出任何其他文字。"
    )
