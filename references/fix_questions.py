...
def build_rewrite_prompt(kp, q_a, q_b, extra_note=""):
    """构建改写提示词: 给出知识点详解与重复题目, 要求改写 q_b 为其他角度。"""
    qtype = q_b.get('type', '')
    type_hints = {
        "single_choice": "单选题必须4个选项(A-D), 有且仅有1个正确答案, answer为单个字母如\"B\"",
        "multiple_choice": "多选题必须至少4个选项(A-D), 设置2-4个正确答案, answer为字母组合如\"ABD\"; 严禁设计成单选式(只有一个正确选项)",
        "true_false": "判断题answer为\"True\"或\"False\"",
        "short_answer": "简答题必须包含reference_answer和scoring_points(3-6个评分要点)",
    }
    lines = []
    lines.append("你是ESG认证考试题库命题与修复专家。题库核查发现两道题目考察角度重复，")
    lines.append("请将其中一道【目标题目】改写为其他考察角度的新题，保留原题。")
    lines.append("")
    lines.append(f"## 知识点: {kp.get('id','')} — {kp.get('topic','')}")
    lines.append(f"模块: {kp.get('module','')} | 难度: {kp.get('level','')}")
    lines.append(f"知识点详解: {kp.get('explanation','')}")
    lines.append("")
    lines.append(f"## 保留的原题: {q_a.get('id','')} [{q_a.get('type','')}]")
    lines.append(f"{q_a.get('question','')}")
    lines.append(f"答案: {q_a.get('answer','')} | 解析: {q_a.get('explanation','')[:100]}")
    lines.append("")
    lines.append(f"## 目标题目(需改写): {q_b.get('id','')} [{qtype}]")
    lines.append(f"{q_b.get('question','')}")
    lines.append(f"答案: {q_b.get('answer','')} | 解析: {q_b.get('explanation','')[:100]}")
    lines.append("")
    lines.append("## 改写要求")
    lines.append(f"1. 题型保持为 {qtype} 不变")
    lines.append(f"2. 保留原题目ID {q_b.get('id','')} 与 kp_ref {q_b.get('kp_ref','')} 不变")
    lines.append(f"3. level保持 {q_b.get('level','')} 不变")
    lines.append("4. 从以下考察角度中任选一个(与保留原题角度不同):")
    for name, desc in REWRITE_STYLES:
        lines.append(f"   - {name}: {desc}")
    lines.append("5. 必须基于知识点详解出题, 确保答案正确, 考察角度不得与保留原题雷同")
    lines.append("6. 题干不得与题库中其他题目高度相似")
    lines.append(f"7. 题型格式硬性要求: {type_hints.get(qtype, '')}")
    lines.append("8. 输出JSON对象格式:")
    if qtype == "multiple_choice":
        lines.append('   {"question": "题干", "options": {"A":"...","B":"...","C":"...","D":"..."}, "answer": "ABD", "explanation": "解析"}')
    elif qtype == "single_choice":
        lines.append('   {"question": "题干", "options": {"A":"...","B":"...","C":"...","D":"..."}, "answer": "B", "explanation": "解析"}')
