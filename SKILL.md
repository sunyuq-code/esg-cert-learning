...
2. **Build/update the knowledge base** at `references/knowledge_base.md`:
   ...
   - **Enrich knowledge point details**: immediately after building the
     knowledge base, run
     `python enrich_knowledge_base.py` (DeepSeek API) so every knowledge
     point gets a detailed explanation (300-500 chars covering definition,
     scope, methods, standards, best practices, common pitfalls). This is a
     mandatory step of first-time construction — thin one-line explanations
     are not acceptable for the learning session.
...
- `references/enrich_knowledge_base.py` — DeepSeek knowledge-point detail enricher
  (mandatory step after first-time knowledge base construction)
